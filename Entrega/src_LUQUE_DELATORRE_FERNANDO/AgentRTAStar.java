...
public class AgentRTAStar extends AbstractPlayer {
	...
	private Types.ACTIONS think() {
		...
		if (heuristics[actual.row][actual.column] < second_minimum.f) {
			// Contamos el tamaño de la "tabla hash" de esta forma
			// Contando solo cuando actualizamos la primera vez
			if (heuristics[actual.row][actual.column] == 0)
				consumo_maximo++;
	...
	public ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
		...
		tAcumulado += (tFin-tInicio);
		System.out.println("Tiempo acumulado: " + tAcumulado/1e6);
